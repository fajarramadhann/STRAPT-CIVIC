...
// import { useAccount } from 'wagmi'; // Not needed for Civic Auth
// import { useXellarWallet } from '@/hooks/use-xellar-wallet'; // Backup - kept for reference

const Index = () => {
  const { isConnected, connectWallet } = useCivicWallet();
  const navigate = useNavigate();
  const [isLoading, setIsLoading] = useState(false);

  // Note: Redirection is now handled by CivicProvider's onSignIn callback
  // This ensures immediate redirection after authentication, not waiting for full wallet connection
  ...
  const handleLaunchApp = async () => {
    ...
    if (isConnected) {
      ...
      setIsLoading(true);
      try {
        // Open Civic Auth connection modal
